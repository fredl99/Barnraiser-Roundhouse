...
if (isset($_POST['submit_delete_avatar'])) {
	...
	elseif (!is_uploaded_file($_FILES['frm_file']['tmp_name'])) {
		$error_txt = _("The picture you are trying to upload is too big. Please upload a picture less than {max_size} in size.");
		$error_txt = str_replace("{max_size}", ini_get('post_max_size'), $error_txt);
		...
		if (isset($_POST['submit_upload_avatar'])) {
			...
			if (!is_dir($destination)) {
				...
				if(!mkdir ($destination, 0770, 1)) {
					$GLOBALS['script_error_log'][] = _("We could not create a directory for your avatars. Please send us a message from the 'contact us' page.");
				}
				umask($oldumask);
			}
