...
if (!isset($_SESSION['user_is_maintainer']) || empty($_SESSION['user_is_maintainer'])) {
	header("Location: /");
	exit;
}
...
elseif (isset($_POST['send_new_password'])) {
	...
	if (!empty($result[0]['user_id'])) {
		...
		$email_subject = _("Here is your new Blog password");
...
elseif (isset($_POST['send_new_verification'])) {
	...
	// email, subject, message
	$email_subject = _("Welcome to your new Blog service!");
...
elseif (isset($_POST['save_config_webspace_names'])) {

	backupConfig();
	
	$core_config['registration']['unauthorized_webspace_names'] = $_POST['unauthorized_webspace_names'];

	writeToConfig('$core_config[\'registration\'][\'unauthorized_webspace_names\']', $core_config['registration']['unauthorized_webspace_names']);
}
...
elseif (isset($_POST['save_config_maintainer_webspace_ids'])) {

	backupConfig();
	
	$core_config['security']['maintainer_webspace_ids'] = $_POST['maintainer_webspace_ids'];

	writeToConfig('$core_config[\'security\'][\'maintainer_webspace_ids\']', $core_config['security']['maintainer_webspace_ids']);
}
...
if (isset($uri_routing[1]) && $uri_routing[1] == "account" && isset($uri_routing[2])) {
	...
	$query = "
		SELECT user_id, user_webspace, user_name, user_email, user_dob, user_live 
		FROM " . $db->prefix . "_user 
		WHERE 
