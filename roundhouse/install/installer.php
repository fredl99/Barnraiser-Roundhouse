...
elseif (isset($_POST['perform_installation'])) {
	...
	// set the session_id -----------------
	if ($core_config['php']['session_name'] == "PHPSESSIDROUNDHOUSE") {
		$php_session_name = 'PHPSESSIDROUNDHOUSE';
	...
	if (isset($_POST['installation_type']) && $_POST['installation_type'] == "multiuser") {
		...
		writeToConfig('$core_config[\'script\'][\'single_webspace\']', $user_webspace);

		$core_domain = $http . "://" . $domain;
	...
	if (empty($GLOBALS['script_error_log'])) {
		...
		if (empty($user_webspace)) {
			$GLOBALS['script_error_log'][] = _("You must provide an blog name.");
		}
		else {
	...
	$email_subject = _("Test email from Roundhouse");
			
	$mail->Subject = utf8_decode($email_subject);

	$email_message_txt = "This is a test message from Roundhouse to confirm that your email configuration is correct.";
	...
	if($mail->Send()) {
		...
		$GLOBALS['script_error_log'][] = _("Either your email server configuration or your email address is incorrect as Roundhouse failed to send you a test email.");
	}
	...
	if (!function_exists('mysql_connect')) {
		$system_check['result'] = _("Roundhouse needs MySQL. Please add MySQL support to PHP.");
	...
	if ( (int) phpversion() < 5) {
		$system_check['is_valid'] = 0;
		$system_check['result'] = _("Roundhouse needs PHP to be version 5.0 or greater. Your version is {version}.");
	...
	if (!function_exists('curl_init') || !function_exists('curl_setopt') || !function_exists('curl_exec')) {
		$system_check['is_valid'] = 0;
		$system_check['result'] = _("Roundhouse needs Curl. Please add Curl to PHP.");
		$is_error = 1;
	}
	else {
	...
	if (!extension_loaded ('bcmath')) {
		$system_check['is_valid'] = 0;
		$system_check['result'] = _("Roundhouse needs BCMath. Please add BCMath to PHP.");
		$is_error = 1;
	}
	else {
	...
	if (!function_exists('gettext')) {
		$system_check['is_valid'] = 0;
		$system_check['result'] = _("Roundhouse needs gettext. Please add gettext to PHP.");
		$is_error = 1;
	}
	else {
	...
	if (function_exists('gd_info')) {
		...
		if (!isset($gd_info['GD Version'])) {
			$system_check['is_valid'] = 0;
			$system_check['result'] = _("Roundhouse needs GD Library. Please add GD Library support to PHP.");
			$is_error = 1;
		}
		else {
